...
| **Database** | Supabase (PostgreSQL + PostGIS), SQLAlchemy, Alembic | Supabase 관리형 PostgreSQL + PostGIS 확장으로 공간 인덱싱 및 위치 기반 고속 조회. SQLAlchemy ORM으로 사용자 이력 관리 |
...
- **Python** 3.12+
- **Node.js** 20+
- **Supabase** 프로젝트 (무료 티어, PostgreSQL + PostGIS)
- **Docker** & **Docker Compose** (선택 - 로컬 DB 사용 시)
...
DATABASE_URL=postgresql+asyncpg://postgres.[PROJECT-REF]:[YOUR-PASSWORD]@aws-0-ap-northeast-2.pooler.supabase.com:5432/postgres
...
2. SQL Editor에서 `CREATE EXTENSION IF NOT EXISTS postgis;` 실행
3. Settings > Database > Connection string (Session pooler, 포트 5432) 복사
4. `backend/.env`의 `DATABASE_URL`에 입력

> 로컬 Docker DB 사용 시: `docker compose --profile local up db`
